...
marsTypes = {}
marsTypes[1] = { life = 2, dmg = 0.5 , scoreKilled = 5  , scoreHit =  1 , subsScore = 2 }
marsTypes[2] = { life = 3, dmg = 1   , scoreKilled = 10 , scoreHit =  2 , subsScore = 4 }
marsTypes[3] = { life = 4, dmg = 1.5 , scoreKilled = 15 , scoreHit =  3 , subsScore = 6 }
	...
	if #marsList > 0 and temp.mBullet >= level[CurrentLevel].vel/2 then	
		...
		for i = 1, numMars do
			...
			addBullet( "mars", marsList[selectedGuy].x + marsList[selectedGuy].w/2, marsList[selectedGuy].y + marsList[selectedGuy].h, 
						marsTypes[marsList[selectedGuy].tGuy].dmg , 1 , marsList[selectedGuy].tGuy )
		end
