...
function addBullet( identityChar , x , y , atk , tGuy )
...
function moveBullets()
	...
	for b = 1, #bts["mars"] do
		...
		if bts["mars"][b].y +  bts["mars"][b].h*wMul >= Screen.downLimit then 
...
function checkBulletCollisionPlayer()
	...
	for b = 1, #bts["player"] do
		...
		elseif ovni.visible then
			if insideBox( bts["player"][b].x, bts["player"][b].y, ovni.mov.x, ovni.mov.y, ovni.dimens.w, ovni.dimens.h) then
				ovni.life = ovni.life - bts["player"][b].dmg
				player.score = player.score + marsTypes[ ovni.tGuy ].scoreHit
				addBulletRemoval( "player", b )	
				if ovni.life <= 0 then
					player.score = player.score + marsTypes[ ovni.tGuy ].scoreKilled
				end
			end
		end
